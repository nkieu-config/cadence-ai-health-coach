    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative flex h-16 w-16 items-center justify-center">
        <svg className="absolute size-full -rotate-90">
          <circle cx="32" cy="32" r="24" className="stroke-muted/40 dark:stroke-muted/20" strokeWidth="3.5" fill="none" />
          <circle cx="32" cy="32" r="24" className={`transition-all duration-500 ${colorClass}`} strokeWidth="3.5" fill="none" strokeDasharray="150.8" strokeDashoffset={strokeDashoffset} strokeLinecap="round" />
        </svg>
...
export function TodaySummary({ checkin, date }: { checkin: Checkin | null; date: string }) {
  ...
  const pillars = [
    { val: checkin?.mealsCount ?? 0, max: 3, color: "stroke-emerald-500", label: "การกิน", text: checkin ? `${checkin.mealsCount}/3 มื้อ` : "--", icon: Utensils },
    { val: checkin?.sleepHours ?? 0, max: 8, color: "stroke-blue-500", label: "การนอน", text: checkin ? `${checkin.sleepHours} ชม.` : "--", icon: Moon },
    { val: checkin?.movementMinutes ?? 0, max: 30, color: "stroke-purple-500", label: "เคลื่อนไหว", text: checkin ? `${checkin.movementMinutes} นาที` : "--", icon: Activity }
  ];
    ...
    <Card className="h-full flex flex-col justify-between">
      <CardHeader className="pb-4 flex flex-row items-start justify-between space-y-0">
        <div className="space-y-1">
          ...
          <CardTitle className="text-lg">บันทึกของวันนี้</CardTitle>
        </div>
      ...
      <CardContent className="space-y-6 flex-1 flex flex-col justify-between">
        <div className="grid grid-cols-3 gap-2 py-2 bg-muted/20 dark:bg-muted/10 rounded-xl">
          {pillars.map((p) => (
            <ProgressCircle key={p.label} value={p.val} max={p.max} colorClass={p.color} label={p.label} valueText={p.text} icon={p.icon} />
          ))}
        </div>
