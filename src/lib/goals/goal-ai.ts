...
export function buildGoalPrompt(
  ...
  const blocks = [
    [
      "ตารางมาตรฐาน (few-shot) — goal ที่ตอบต้องอยู่ใน **เรื่องเดียวกับ** goal_มาตรฐาน ของ situation นั้น:",
      JSON.stringify(fewShot, null, 2),
      "ปรับได้: ถ้อยคำ · วันที่อ้างถึง · จำนวนครั้ง/นาที — ให้เข้ากับข้อมูลจริงของผู้ใช้",
      "ห้ามเปลี่ยนเรื่อง: ถ้า goal_มาตรฐาน ของ situation นั้นเป็นเรื่องพัก ห้ามตอบเป็นเรื่องมื้อเช้า/การนอน",
    ].join("\n"),
  ];
  ...
  if (constraints) {
    blocks.push(
      [
        ...
        "ถ้าอ้างถึงวัน ให้ใช้วันจากตารางนี้เท่านั้น ห้ามเดาวันเอง และต้องใช้ให้ตรงความหมายของแต่ละแถว:",
        '"วันที่ตารางแน่นสัปดาห์หน้า" = วันที่ผู้ใช้เพิ่งบอกว่าจะยุ่ง — ใช้วางแผน goal สัปดาห์หน้าเป็นหลัก',
        '"วันที่ต้องตื่นเช้า" = อ้างได้เฉพาะ goal ที่เกี่ยวกับมื้อเช้าหรือการเตรียมตัวตอนเช้าเท่านั้น',
      ].join("\n")
    );
  }
