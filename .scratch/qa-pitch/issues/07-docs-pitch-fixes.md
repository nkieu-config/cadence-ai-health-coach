# QA-07: แก้เอกสารให้ตรงโค้ดก่อน pitch (รอบ audit ทุกบรรทัด)

Status: ready-for-agent
Owner: A
Sprint: 3
Priority: M — เอกสารที่กรรมการอ่าน ห้ามมีข้อมูลผิด/อ้างเกินจริง
Refs: QA-06 (รอบแรก), รายงานเต็ม [docs-audit-2026-07-23.md](../docs-audit-2026-07-23.md)

## ที่มา

QA-06 กวาดเอกสารรอบแรก (20 ก.ค.) เน้นโควตา/โมเดลหลัง INFRA-23 · รอบนี้ตรวจ **ทุกบรรทัด เทียบกับโค้ดจริง** เจอ drift ชุดใหม่ (INFRA-27, F4-06, สถานะ) + จุดที่เอกสารอ้างเกินสิ่งที่ระบบทำ + ตัวเลข QA-02 ที่เพิ่งเก็บจริง

## งาน (เรียงตาม P0 → P1)

- [ ] docs/03 — pattern "พลังงานบ่ายต่ำ" (ไม่มีจริง) → "เครื่องดื่มหวาน" · "~2 นาที" → ตัวเลข QA-02 จริง
- [ ] docs/04 — FR-1.2 ตัวเลขจริง + FR-1.3 "1 วัน"→"30 วัน" + FR-3.1 pattern ที่มีจริง
- [ ] docs/08 — training data (ก๊อปถ้อยคำจากแอป) · "guardrail ทุก request" → ตามจริง · disclaimer ยกผิดคำ
- [ ] docs/10 — QA-03 ไม่เป็นศูนย์ · ตัด countdown · ตัวนับงาน · F4-06 · เพิ่ม docs/12 ในตารางส่งงาน
- [ ] demo-script — 7 จุด (ลำดับ check-in, screenshot, cache, F2-04, แบนเนอร์ reflection, ปุ่มล้างประวัติ)
- [ ] deck-outline — เวลา 9→8 นาที · "ข้อมูลจริง"→"seed จำลอง+dogfood" · F2-04/cache
- [ ] BOARD.md — countdown · ตัวนับงาน · สถานะ INFRA-27/F0-04
- [ ] (P1) docs/07 prompt · docs/06 proxy/auto→ปุ่ม · docs/09 สถานะ+risk · docs/11 markdown/e2e · docs/05 SQL

## กติกาที่ยึด

- **แก้เอกสารให้เล่าความจริง ไม่แก้โค้ดให้ตรงเอกสาร** — โค้ดส่วนใหญ่ดีกว่าสเปก (30>1 วัน, กดปุ่ม>เผาโควตา)
- **ห้ามแตะ prompt ทุกตัว** — หลักฐาน safety 20/20 จะเป็นโมฆะ (บทเรียน INFRA-23)
- งานแตะโค้ด (theme.spec เข้า CI, อุ่น insight 7 วัน) แยก issue

## Comments

---

23 ก.ค. (A) — เปิดจากรายงาน audit รอบ 2 · ตัวเลข QA-02 เก็บครบแล้ว (วันปกติ 1:26 · หนักสุด 1:30 · ผ่าน AC ทั้งคู่)
