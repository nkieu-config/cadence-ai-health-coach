...
>
> ⚠️ **แผงล่างต้องรีเซ็ต `scrollTop = 0` ทุกครั้งที่เปลี่ยนขั้น และมีเงาบอกเมื่อยังมีเนื้อหาต่อ** — React ใช้ DOM node เดิมข้ามขั้น ตำแหน่งเลื่อนจึงติดมาด้วย ผู้ใช้ที่ปัดลงในขั้นก่อนหน้าจะเข้าขั้นถัดไปมาเจอ**กลางรายการ** โดยตัวเลือกแรกอยู่เหนือขอบและปุ่ม "ถัดไป" อยู่ใต้ขอบ ไม่มีสัญญาณสักทาง · ตัวเลือกในขั้น "ข้อจำกัด" คือ **Required Input ของโจทย์ §5.1** (ไม่มีเวลา/ไม่มีสถานที่/งบจำกัด/พักผ่อนไม่พอ) มองไม่เห็น = เก็บข้อมูลไม่ครบ = Personalization อ่อนลง
>
...
>
> ⚠️ **ทุกโซนที่เลื่อนเองในหน้าแชทต้องมี `overscroll-contain`** — ถ้าไม่ใส่ พอเลื่อนบทสนทนาจนสุดบน เบราว์เซอร์จะส่งการเลื่อนต่อไปให้ทั้งหน้า (scroll chaining) แล้ว**หัวข้อกับปุ่มล้างประวัติจะเลื่อนหายไปด้วย** ทั้งที่ผู้ใช้ตั้งใจเลื่อนแค่ในกล่อง · `min-h-48` เป็นพื้นความสูงของ shell — ต่ำกว่านี้กล่องจะเตี้ยจนใช้ไม่ได้ แต่สูงกว่านี้จะดันหน้าให้ล้นบนจอแนวนอน (390px) ซึ่งเป็นเหตุให้เกิด chaining ตั้งแต่แรก
>
> **บรรทัด safety อยู่ใต้ layout ทุกหน้ารวมทั้ง `/coach`** — เคยลองย้ายเข้าไปไว้บนสุดของสายสนทนาเพื่อคืนพื้นที่ 48px แต่ **บทสนทนาเปิดมาเลื่อนอยู่ล่างสุด คำเตือนจึงไม่เคยอยู่ในสายตา** บนหน้าที่คนถามเรื่องอาการมากที่สุด · เอากลับมาไว้ล่างตามเดิม พื้นที่ที่ต้องการหาจากที่อื่นแทน (รวมหัวข้อกับปุ่มล้างประวัติเข้าแถวเดียวกันได้คืน 44px) · **ค่า chrome ที่หักในสูตรความสูงรวมบรรทัดนี้ไว้แล้ว — ย้ายเมื่อไหร่ต้องแก้ค่านั้นด้วย**
>
...
**หลักการหน้าแชท** (message-variants.tsx): ข้อความโค้ช = avatar `MessageCircle` + ชื่อ "โค้ช" เป็น**แถวหัวด้านบน** แล้วเนื้อความกินเต็มความกว้าง ไม่มีพื้นฟอง (ตัวตนโค้ชชัด ตามโจทย์ Feature 4) · **เคยวาง avatar เป็นคอลัมน์ซ้าย แต่มันกินความกว้างบรรทัดไป 42px ทุกบรรทัด** ทำให้แชทเหลือ 33 ตัวอักษร/บรรทัดบน iPhone 14 ซึ่งแคบที่สุดในแอป (หน้าอื่น 41–45 · ตอนนี้แชทได้ 39) — บรรทัดสั้นบวก leading โปร่งทำให้อ่านแล้วรู้สึกตัวใหญ่เทอะทะ ทั้งที่ px เท่าหน้าอื่นที่ใช้ `text-base` · ข้อความผู้ใช้ = ฟองเขียวชิดขวา · คำตอบโค้ช render markdown ผ่าน `FormattedMessage` (เขียนเอง ไม่มี library ไม่มี raw HTML) · ช่องพิมพ์เป็น textarea โตตามเนื้อหา Enter ส่ง Shift+Enter ขึ้นบรรทัด · **โควตาหมด = โทน muted อบอุ่น ไม่ใช่ destructive** (โจทย์ข้อ 8 ห้ามกดดัน) · ป้ายโควตาโชว์เฉพาะเมื่อเหลือ ≤ 2
...
| **ย่อหน้ายาว** (reflection, บทความ) | `text-base` (16px) | ตัวไทยดูเล็กกว่า Latin ที่ px เท่ากัน · ย่อหน้าอ่านยาวบนมือถือต้องการ 16px · **ยกเว้นบทสนทนาในหน้าแชท** ดูแถวถัดไป |
| **บทสนทนาในหน้าแชท** | `text-sm` (14px) | เคยเป็น 16px ตามกฎย่อหน้ายาว แต่มันไปนั่งเหนือปุ่ม `text-sm` ในการ์ดใบเดียวกันห่างไม่ถึง 20px — **ต่างกัน 2px ในระยะสายตาเดียว ตาจับได้ทันทีว่าใหญ่ผิดส่วน** · เทียบข้ามหน้าไม่มีใครจำได้ว่าหน้าอื่นใช้ขนาดไหน แต่เทียบในกรอบเดียวกันเห็นทุกครั้ง · **`textarea` ยังต้องเป็น `text-base` (16px)** ห้ามลด ไม่งั้น iOS ซูมหน้าเองตอนโฟกัส |
| line-height | ตั้งใน `@theme` แล้ว | ไทยมี 4 ชั้นต่อบรรทัด (วรรณยุกต์/สระบน/ตัว/สระล่าง) — เนื้อความ (`text-sm` และ `text-base`) ใช้ **1.65** เท่ากัน **ไม่ต้องใส่ `leading-*` เอง** · `text-base` เคยเป็น 1.75 แต่มันไปตกกับคอลัมน์ที่**แคบที่สุด**ในแอป (แชท 324px · สรุปสัปดาห์ 278px) ซึ่งกลับกับหลักที่ว่าบรรทัดยิ่งยาวยิ่งต้องการ leading มาก |
