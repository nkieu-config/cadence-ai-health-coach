...
>
> ⚠️ **หน้านี้เป็นหน้าเดียวที่กำหนดความสูงเอง จึงพังเงียบได้** — ถ้าค่า chrome ที่หักไว้น้อยกว่าของจริง หน้าจะยาวเกินจอ แล้ว**เมนูล่างจะไปทับช่องพิมพ์** ไม่ใช่แค่ layout เพี้ยน · ค่าที่หักต้องรวม `env(safe-area-inset-top)` **และ** `env(safe-area-inset-bottom)` (เมนูล่างบวก inset ล่างเข้าไปเอง) · แก้ chrome เมื่อไหร่ต้องแก้ค่านี้ด้วยเสมอ
>
> ⚠️ **แผงล่างของการ์ดแชทต้องเลื่อนเองได้** (`max-h-[calc(100%-6rem)] overflow-y-auto`) — flow ตั้งเป้าหมายทำให้แผงสูงกว่าการ์ดทั้งใบบนมือถือจอเตี้ย ถ้าไม่ให้เลื่อน `overflow-hidden` ของการ์ดจะตัดปุ่ม "ถัดไป" หายไปเฉย ๆ จนกดต่อไม่ได้ · เพดานเขียนเป็น `100% - 6rem` ไม่ใช่สัดส่วน — รับประกันบทสนทนา 96px และไม่ไปบีบแผงบนจอที่กว้างพอ
>
> **บรรทัด safety บนหน้าแชทอยู่ในสายสนทนา ไม่ใช่ใต้ layout** — `LayoutSafetyNotice` คืน `null` ที่ `/coach` แล้วให้ `chat-client` วาดเองเป็นชิ้นแรกในโซนที่เลื่อนได้ · เหตุผล: ทุกหน้าอื่นเนื้อหายาวกว่าจอ บรรทัดนี้จึงไปนั่งท้ายเอกสารและไม่กินพื้นที่ที่มองเห็น แต่ `/coach` สูงเท่าจอพอดี บรรทัดเดียวกันจึงกิน **48px จากช่องบทสนทนาที่เหลือ 103px ที่ 320px** · ย้ายแล้วช่องบทสนทนาเป็น 151px (+47%) โดยคำเตือนยังขึ้นครบทุกหน้าตามโจทย์ข้อ 7.9 · **เพิ่ม/ลบหน้าที่วาดเอง ต้องแก้ทั้ง `RENDERS_ITS_OWN` และค่า chrome ที่หักในสูตรความสูง**
>
> ทั้งสองข้อมีด่าน `e2e/coach.spec.ts` คุมแล้ว (จอ 320 + มือถือ) — ด่าน overflow ใน `checks.ts` เช็กแค่แนวนอน จับเรื่องนี้ไม่ได้ และ `isVisible()` ตอบ true ให้ของที่ถูก `overflow` ตัดหาย
