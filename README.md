...
An AI wellness coach for students and early-career professionals. Cadence helps people notice patterns
in eating, sleeping, and movement, then turn those patterns into practical micro-goals that fit their
real week.
...
[Project summary](docs/summary/showcase-en-light.pdf) ·
...
> [!NOTE]
> **The app UI is in Thai.** This README uses English descriptions, and a browser translation extension
> can be used when exploring the live demo.
...
- **Project summary** — [9-page overview](docs/summary/showcase-en-light.pdf) in English.
...
messages per day because it uses Gemini's free-tier quota.

## About the project

Cadence is a wellness coach, not a medical service. It is designed for students and early-career
professionals whose routines are shaped by classes, deadlines, commuting, and changing energy levels.
...
The check-in takes under three minutes, uses tap-based answers, and shows follow-up questions only when
they are relevant. The coach uses the user's own records to make suggestions that are small enough to
fit their actual week.
...
- **Pattern insights** — identify relationships between health behaviours and real-life schedule constraints.
- **AI coach** — start with a question drawn from your records, continue the conversation, and turn it into a guided goal.
- **Micro-goals** — choose up to two small weekly goals and track progress day by day.
- **Weekly reflection** — compare recent weeks and reflect on what changed.
...
- **Responsible AI design** — calculates evidence separately from AI-generated language and sets clear boundaries for coaching. See [AI design](docs/07-ai-design.md).
- **Privacy and quality** — keeps health data visible only to its owner, supports account deletion, sends no name or email to the model, and verifies behaviour with unit and end-to-end tests. See [safety and privacy](docs/08-safety-privacy.md).

## Project context

Cadence was built by a team of four in four weeks for CSTU Spark Camp in AI 2026, with a zero-dollar
budget. The team designed, implemented, tested internally, and deployed a working demo for the final
