...
Migration อยู่ที่ `supabase/migrations/` (`0001`→`0003`) — A รันบน project กลางให้แล้ว เพื่อนแค่ `git pull` ก็ใช้ได้

**ก่อนเปิด PR — รันเองก่อน จะได้ไม่ต้องรอ CI แดง:**
...
CI บังคับ **ทั้งสองด่าน** ทุก PR — `e2e` ไม่ใช่ของเสริม มันเปิดทุกหน้าจริงบนมือถือ+เดสก์ท็อป × light+dark แล้วเช็ค h1 อันเดียว · ไม่มี horizontal scroll · ปุ่ม ≥ 44px · contrast ≥ 4.5:1 · ไม่มี console error · เช็คอินบันทึกได้จริง

> unit test ครอบแค่ตรรกะใน `lib/` — **PR ที่ทำ layout พังหรือ dashboard ขาว จะผ่าน `verify` เขียวหมด** `e2e` คือด่านเดียวที่จับได้

**กฎ UI:** ส่วนแรกของ [DESIGN.md](DESIGN.md)
...
`AGENTS.md` / `CLAUDE.md` = กติกาสำหรับ AI coding agent · ไฟล์ config ที่เหลือใน root อยู่ตำแหน่งบังคับของ npm/Next/TS/ESLint/Tailwind/shadcn — อย่าย้าย
...
- 1 issue = 1 branch = 1 PR + review ≥ 1 คน · claim งานโดยแก้ `Status:` ในไฟล์ issue

กติกาที่เหลือ (sync main, โซนไฟล์, บัญชี demo) อยู่ใน [.scratch/BOARD.md](.scratch/BOARD.md)
