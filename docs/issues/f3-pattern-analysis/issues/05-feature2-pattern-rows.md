...
2026-07-14 (A): **เปิด issue นี้ย้อนหลัง — ทำงานก่อนแล้วค่อยเปิด issue ซึ่งผิดลำดับ** · เจอตอนจะเขียน seed (INFRA-06)

**ข้อจำกัดที่ยังจริง:** candidate ที่ไม่มี template จะถูก `.filter()` ทิ้งเงียบ ๆ ไม่ขึ้นหน้าจอ (เกิดกับ `eating-on-time-energy` มาแล้ว) — ตอนนี้มีเทสต์บังคับให้ CI แดงถ้าเกิดซ้ำ

**หมายเหตุถึงคนรีวิว:** issue นี้แตะ `lib/patterns` ที่ merge ไปแล้ว (F3-01) — เป็นการ**เพิ่ม** candidate ไม่ได้แก้ของเดิม
