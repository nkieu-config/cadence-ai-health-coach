...
- legend สัญลักษณ์: มือถือพับใน `<details>` "สัญลักษณ์ปัจจัยรบกวน" (summary `min-h-11` มี focus ring) · `lg:` กางเป็นแถวเดียว
...
1. **หัวเรื่อง** — h1 + คำอธิบาย "รับคำแนะนำเพื่อสร้างนิสัย…" ที่ **`hidden lg:block`** (มือถือคืนพื้นที่ 42px) · **ส่งเข้าไปเป็น prop `heading` ของ `CoachChatClient`** แล้วไปนั่งแถวเดียวกับปุ่มล้างประวัติ — เดิมเป็นบล็อกแยกเหนือกล่องแชท ซึ่งทำให้ปุ่มล้างประวัติต้องมีแถวของตัวเองอีกแถว (กิน 44px + gap 16px ไปกับปุ่ม ghost ปุ่มเดียว)
2. **ชั้นนอก** = ตัวคุมความสูง `h-[calc(100dvh-14rem-env(safe-area-inset-top,0px)-env(safe-area-inset-bottom,0px))] min-h-48 flex flex-col` · **`lg:h-[calc(100dvh-8.5rem)]`** — เดสก์ท็อปหักน้อยกว่าเพราะไม่มี header sticky และไม่มีเมนูล่าง · **14rem = header 61 · `py-6` 48 · safety notice 48 · เมนูล่าง 59** (h1 ไม่อยู่ในสูตรแล้วเพราะย้ายเข้าไปในกล่อง) · **`min-h-48` ไม่ใช่ `min-h-64`** — 64 ดันหน้าให้ล้นบนมือถือแนวนอน (จอสูง 390px) แล้วเกิด scroll chaining · ต้องหัก inset **ทั้งบนและล่าง** เพราะเมนูล่างบวก inset ล่างเข้าไปเอง — ดู DESIGN.md ข้อยกเว้นหน้าแชท
3. **แถบบนสุด** `min-h-11 shrink-0 flex items-center justify-between` — ซ้าย = หัวเรื่อง (h1 + คำอธิบาย) · ขวา = ปุ่มล้างประวัติ · **ป้ายโควตาย้ายไปอยู่เหนือช่องพิมพ์** ซึ่งเป็นจุดที่มันมีความหมายจริง
4. **ชั้นกลาง** = กล่องการ์ด `flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-card` แบ่งอีก 2 ส่วน:
   - **รายการข้อความ** `role="log"` `flex min-h-0 flex-1 overflow-y-auto overscroll-contain p-5` — **ส่วนเดียวที่เลื่อน** · `overscroll-contain` กันไม่ให้การเลื่อนจนสุดบนส่งต่อไปเลื่อนทั้งหน้า (จะพาหัวเรื่องกับปุ่มล้างประวัติหายไปด้วย)
   - **แผงล่าง (ปักติดขอบล่างของการ์ด)** `data-slot="chat-panel"` `shrink-0 max-h-[calc(100%-6rem)] overflow-y-auto overscroll-contain border-t p-5 bg-muted/10` ห่อด้วย `relative` ที่มี**เงาไล่สีขอบล่าง**โผล่เฉพาะตอนยังมีเนื้อหาต่อ — ใส่ chip / error / retry / notice โควตา / ฟอร์มพิมพ์ หรือสลับเป็น UI ของ guided flow ทั้งแผง · **เพดาน + `overflow-y-auto` บังคับ** เพราะแผง guided flow สูงกว่าการ์ดทั้งใบบนมือถือจอเตี้ย ถ้าไม่ให้เลื่อน `overflow-hidden` ของการ์ดจะตัดปุ่มหายไปเฉย ๆ · เขียนเป็น `100% - 6rem` ไม่ใช่สัดส่วน เพื่อ**รับประกันบทสนทนา 96px** และให้เพดานทำงานเฉพาะตอนจำเป็น — บนเดสก์ท็อปแผงยังสูงเต็มตามเนื้อหา ภาพในเด็คจึงไม่เปลี่ยน
...
6. **auto-scroll** — เข้าหน้าครั้งแรกเลื่อนแบบ `instant` หลังจากนั้น `smooth` ทุกครั้งที่ `messages` / `isPending` / `guidedStep` เปลี่ยน · **แผงล่างรีเซ็ต `scrollTop = 0` ทุกครั้งที่เปลี่ยนขั้น** เพราะ React ใช้ DOM node เดิม ตำแหน่งเลื่อนเดิมจึงติดมาจนผู้ใช้เข้าขั้นใหม่มาเจอกลางรายการ

**ฟองข้อความ — โค้ชกับผู้ใช้ต่างกันยังไง**

| | ผู้ใช้ | โค้ช |
| --- | --- | --- |
| ตำแหน่ง | ชิดขวา `flex justify-end` | ชิดซ้าย เต็มความกว้าง |
| avatar | ไม่มี | วงกลม `size-8 bg-primary/10` + `MessageCircle` (`aria-hidden`) — **อยู่แถวหัวคู่กับชื่อ ไม่ใช่คอลัมน์ซ้าย** (คอลัมน์ซ้ายกินความกว้างบรรทัด 42px ทุกบรรทัด) |
...
| ความกว้าง | `max-w-[80%]` | เต็มความกว้างการ์ด |
| ขนาดตัวอักษร | `text-sm` | `text-sm` — **ไม่ใช่ `text-base`** ดู DESIGN.md แถว "บทสนทนาในหน้าแชท" |
...
- ✅ `loading.tsx` ซิงก์กับหน้าจริงแล้ว (`14rem` / `lg:8.5rem` / `min-h-48` / หัก inset ทั้งบนล่าง / แถวหัว + byline ทรงเดียวกัน) — **แก้ค่าความสูงในหน้าจริงเมื่อไหร่ ต้องแก้ไฟล์นี้ด้วยทุกครั้ง**
...
- **โค้ชทักก่อนด้วยข้อมูลจริง** — "ช่องแชทเปล่า + จุดกระพริบ = ภาพจำของ AI ทั่วโลก" และ "โยนภาระคิดคำถามให้ผู้ใช้ ทั้งที่โจทย์ Feature 4 บอกว่าหัวใจคือคำถามที่โค้ชถาม" · ตัวเลข/วันคำนวณในโค้ด ไม่ให้ LLM คิด ตามกฎเหล็กใน [agents.md](agents.md)
...
4. CTA คอลัมน์เดียว `max-w-xs` กว้างเท่ากันทั้งหมด: "ลองเลย ด้วยบัญชีตัวอย่าง" (primary ไอคอน `Play`) บนสุด → บรรทัดอธิบาย `text-xs text-muted-foreground` → เส้นคั่น "หรือ" → "เข้าสู่ระบบ" (secondary — outline จมหายไปกับพื้นขาวของหน้านี้ ไม่มีการ์ดรองเหมือนหน้า login) / "สมัครสมาชิก" (outline)
...
- ป้าย: "การกิน" · "การนอน" · "การเคลื่อนไหว" · ปุ่ม: "ลองเลย ด้วยบัญชีตัวอย่าง" / "กำลังเปิดบัญชีตัวอย่าง…" · "เข้าสู่ระบบ" · "สมัครสมาชิก"
- "เปิดบัญชีตัวอย่างที่มีข้อมูลจริง 4 สัปดาห์ ไม่ต้องสมัคร" · "หรือ"
...
- ปุ่มบัญชีตัวอย่างเป็น `<form>` + server action ไม่ใช่ `<Link>` — ระหว่างส่งจะ `disabled` และเปลี่ยนข้อความผ่าน `useFormStatus`
...
2. CardContent `space-y-4`: แถบยืนยันเมื่อ `?deleted=1` (ลบบัญชีสำเร็จ) → แถบ error OAuth (เมื่อ `?error=oauth`) → แถบ error บัญชีตัวอย่าง (เมื่อ `?error=demo`) → ปุ่มบัญชีตัวอย่าง (outline) → ปุ่ม Google เต็มกว้าง → เส้นคั่น "หรือ" → ฟอร์มอีเมล/รหัสผ่าน → ปุ่ม submit → ลิงก์ไป `/register`
...
| บัญชีตัวอย่างเข้าไม่ได้ | `?error=demo` — ไม่ได้ตั้ง `DEMO_EMAIL`/`DEMO_PASSWORD` บน server หรือรหัสไม่ถูก | "เปิดบัญชีตัวอย่างไม่สำเร็จ — เข้าด้วยอีเมลด้านล่างได้ตามปกติ" | [actions.ts:43-58](../src/lib/auth/actions.ts#L43) |
...
- "เข้าสู่ระบบ" · "Cadence — ผู้ช่วยดูแลสุขภาพประจำวัน" · "ลองเลย ด้วยบัญชีตัวอย่าง" / "กำลังเปิดบัญชีตัวอย่าง…" · "ดำเนินการต่อด้วย Google" · "หรือ" · "อีเมล" · "รหัสผ่าน" · ปุ่ม "เข้าสู่ระบบ" / "กำลังเข้าสู่ระบบ…" · "ยังไม่มีบัญชี? สมัครสมาชิก"
...
- ปุ่มบัญชีตัวอย่างอยู่บนสุดเพราะกลุ่มเป้าหมายของเรโปตอนนี้คือ recruiter ที่เปิดจาก README — ทางเข้าที่กดครั้งเดียวถึงหน้าจอจริงต้องมาก่อนช่องกรอก · รหัสผ่านอ่านจาก env ฝั่ง server เท่านั้น (ห้ามใช้ `NEXT_PUBLIC_`)
...
6. ✅ **ปิดแล้ว — copy ปุ่มผิดบริบทในหน้า backfill** — แต่ละ variant ส่ง `submitLabel` ของตัวเอง แบบเดียวกับที่ส่ง `heading` อยู่แล้ว — ปุ่มสุดท้ายเขียน "บันทึกเช็คอินวันนี้" hardcode ในแกน `checkin-form.tsx` → หน้า `/checkin/edit/[date]` ที่กรอกย้อนหลังก็ยังขึ้นคำว่า "วันนี้"
...
10. ✅ **ปิดแล้ว — nudge เป็น `<Link>` จัดสไตล์เอง** — ได้ `min-h-11` + focus ring แล้ว — [page.tsx:26](../src/app/(app)/checkin/page.tsx#L26) ไม่ได้ใช้ `buttonVariants()`/`min-h-11` ความสูงมาจาก padding + 2 บรรทัด
...
19. ✅ **ปิดแล้ว — `##` ยังไม่ถูกแปลง** — F4-06 ปิดด้วยทางเลือกที่ 3 (แปลงฝั่งแสดงผล ไม่ลง lib ไม่แตะ prompt) · หัวข้อเรนเดอร์เป็นบรรทัดหนา ไม่ใช่ `<h*>` จริง เพราะฟองแชทอยู่ในหน้าที่มีลำดับหัวข้อของตัวเองแล้ว — `FormattedMessage` แก้ `**` และ bullet แล้ว แต่ไม่ได้จัดการ `##` (ตรงกับ f4/06 ที่ยังค้างใน BOARD)
...
27. ✅ **ปิดแล้ว — skeleton ไม่ตรงทรง** — `/coach` แก้ความสูงแล้ว · `/goals` ได้ `GoalsSkeleton` (ปุ่มขอเป้า + ตารางติ๊ก 7 ช่อง) — `/coach` แก้ความสูงให้ตรงแล้ว (`17.75rem` + safe-area) · **`/goals` ยังใช้ `TextPageSkeleton`** ที่ไม่ตรงกับการ์ดขอเป้า + ตารางติ๊ก 7 ช่อง
...
30. ✅ **ปิดแล้ว — h1 ของหน้า auth ผิดสเกล** — ใส่ `text-xl font-semibold lg:text-2xl` บน h1 เองทั้ง 4 หน้า — ซ้อนใน `CardTitle` จึงได้ `text-base font-medium` แทน `text-xl font-semibold lg:text-2xl` (หน้า `/forgot-password` กับ `/reset-password` ที่เพิ่มใหม่ใช้ทรงเดียวกัน จึงมีอาการเดียวกัน)
31. ✅ **ปิดแล้ว — ไม่มีการ log error ที่ไหนเลย** — error boundary ทั้งสองชั้น log ผ่าน `useEffect` แล้ว — `error.tsx` ทั้งสองไฟล์ยัง destructure เฉพาะ `reset` · หมายเหตุ: server action มี `console.error` แล้วใน `generateInsight` · `generateReflection` · `deleteAllData`
32. ⬜ **ยังค้าง (ตั้งใจ) — ไม่มี `global-error.tsx`** — ไม่มีใครสั่ง ไม่อยู่ใน FR หรือ §7 ของบรีฟ · การสร้างหน้าใหม่ตอนใกล้ freeze คือความเสี่ยงที่ไม่คุ้ม
33. ⬜ **ยังค้าง (ตั้งใจ) — ไม่มี UI แก้โปรไฟล์เลยทั้งแอป** — ผู้ใช้แก้ชื่อเล่น/สถานะ/ข้อจำกัดไม่ได้หลัง onboarding · **ไม่มี FR ข้อไหนขอ** และ F0-02 เลื่อนไป F7 แต่ F7 ไม่เคยเปิด issue ให้ = ไม่เคยอยู่ในแผนที่จะส่ง · เป็นงาน "ถ้าไปต่อ" ไม่ใช่ของที่ตกหล่น
34. ✅ **ปิดแล้ว — skeleton `/settings/privacy` ไม่ตรงทรง** — ขอ 7 การ์ดตามหน้าจริง — ใช้ `TextPageSkeleton` 3 การ์ด ทั้งที่หน้าจริงมี 7 การ์ด
...
37. ✅ **ปิดแล้ว — onboarding checkbox รับทราบไม่มีสไตล์ focus** — เพิ่ม `has-focus-visible:ring-3` · ยืนยันด้วยการกด Tab จริงบนบัญชีทดสอบ (โฟกัสแบบโปรแกรมไม่ทริกเกอร์ `:focus-visible` จึงเช็คจาก CSS อย่างเดียวไม่ได้) — ยังมีแค่ `has-checked:` ไม่มี `has-focus-visible:` (เป็นด่านบังคับ FR-0.3 จึงควรเก็บ)
38. ✅ **ปิดแล้ว — onboarding กลุ่ม Chip ไม่ได้ผูกกับ label** — ทั้ง 4 กลุ่มใช้ `<ChipGroup>` ที่มี `role="group"` + `aria-labelledby` — หน้าเช็คอินแก้แล้ว (ข้อ 9) แต่ onboarding ยังไม่ได้แก้
39. ⬜ **ยังค้าง (ตั้งใจ) — landing / login / register ไม่มี `loading.tsx`** — รวมถึง `/forgot-password` และ `/reset-password` · หน้าเหล่านี้เบาและ redirect เร็ว การเพิ่มไฟล์ใหม่ตอนใกล้ freeze ให้ผลน้อยกว่าความเสี่ยง — บันทึกเป็นข้อยกเว้นที่รู้ตัว
