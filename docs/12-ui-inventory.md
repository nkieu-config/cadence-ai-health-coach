...
1. **หัวเรื่อง** `shrink-0` — h1 + คำอธิบาย "รับคำแนะนำเพื่อสร้างนิสัย…" ที่ **`hidden lg:block`** (มือถือคืนพื้นที่ 42px ให้บทสนทนา)
2. **ชั้นนอก** = ตัวคุมความสูง `h-[calc(100dvh-16.75rem-env(safe-area-inset-top,0px)-env(safe-area-inset-bottom,0px))] min-h-64 flex flex-col` · **`lg:h-[calc(100dvh-13rem)]`** — เดสก์ท็อปหักน้อยกว่าเพราะไม่มี header sticky และไม่มีเมนูล่าง · **16.75rem = chrome จริงที่วัดได้ 261px + เผื่อ 7px** (header 61 · `py-6` 48 · h1 29 · `gap-4` 16 · safety notice 48 · เมนูล่าง 59) · ต้องหัก inset **ทั้งบนและล่าง** เพราะเมนูล่างบวก inset ล่างเข้าไปเอง — ดู DESIGN.md ข้อยกเว้นหน้าแชท
...
4. **ชั้นกลาง** = กล่องการ์ด `flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-card` แบ่งอีก 2 ส่วน:
   ...
   - **แผงล่าง (ปักติดขอบล่างของการ์ด)** `data-slot="chat-panel"` `shrink-0 max-h-[calc(100%-4rem)] overflow-y-auto border-t p-4 bg-muted/10` — ใส่ chip / error / retry / notice โควตา / ฟอร์มพิมพ์ หรือสลับเป็น UI ของ guided flow ทั้งแผง · **เพดาน + `overflow-y-auto` บังคับ** เพราะแผง guided flow สูงกว่าการ์ดทั้งใบบนมือถือจอเตี้ย ถ้าไม่ให้เลื่อน `overflow-hidden` ของการ์ดจะตัดปุ่มหายไปเฉย ๆ · เขียนเป็น `100% - 4rem` ไม่ใช่สัดส่วน เพื่อ**รับประกันบทสนทนา 64px** และให้เพดานทำงานเฉพาะตอนจำเป็น — บนเดสก์ท็อปแผงยังสูงเต็มตามเนื้อหา ภาพในเด็คจึงไม่เปลี่ยน
