...
>
> อัปเดตเมื่อ 23 ก.ค. 2026 · ครอบคลุม 14 route + โครงร่วม · ทุกหัวข้อลิงก์ไปโค้ดจริง กดดูได้
> screenshot ของทุกหน้า (light/dark × มือถือ/เดสก์ท็อป) อยู่ที่ [docs/pitch/screenshots/](pitch/screenshots/) สร้างใหม่ได้ด้วย `npm run shots`
...
- หน้านอกแอป — landing · เข้าสู่ระบบ · สมัคร · onboarding · error/not-found
...
**กราฟใช้ `--chart-1` ถึง `--chart-6`** — นอน = 1 (ฟ้า) · กิน = 2 (ส้ม) · ขยับ = 3 (เขียว) · ผูกโหมดมืดให้แล้ว
...
**z-index ทั้งแอปมี 2 ชั้นเท่านั้น** — `z-10` แถบ sticky (header มือถือ) · `z-50` ของที่ต้องลอยเหนือทุกอย่าง (popover ปัจจัยรบกวน, skip link)
...
8. **ห้ามใช้สีเตือนภัยกับพฤติกรรมผู้ใช้** — `--destructive` สงวนไว้ให้ **error ของระบบ** (บันทึกไม่สำเร็จ เชื่อมต่อไม่ได้) ไม่ใช่ "คุณนอนน้อย" · นอน 4 ชม. กับ 8 ชม. ใช้สีเดียวกัน ความหมายอยู่ที่ความยาวแท่ง · พลังงานต่ำใช้ `outline` · ไม่มีคะแนน ไม่มีเปอร์เซ็นต์รวม ไม่มีเกรด — โจทย์ข้อ 8 สั่งห้ามกดดัน
...
| loading.tsx | มี → `FormSkeleton` = กล่อง 448px + `LoadingLabel` + การ์ด 1 ใบ (หัว 2 แถบ + 3 กลุ่ม label/ชิปสูง 44px) [page-skeleton.tsx:47](../src/components/page-skeleton.tsx#L47) · **skeleton ใช้ container ฟอร์ม 448px แต่หน้าจริง `width="content"` → ความกว้างขยับตอนโหลดเสร็จบนเดสก์ท็อป** |
...
| 4 บริบทวัน | พลังงานวันนี้โดยรวม | ต่ำ / กลาง / สูง | ✅ | เสมอ |
...
| loading.tsx | มี → `TextPageSkeleton` (การ์ดข้อความ 3 ใบเรียงลงมา) · **หน้าจริงเป็น grid 2–3 คอลัมน์บนจอกว้าง → รูปทรงไม่ตรง** |
| Suspense | **ไม่มี** |
...
3. **รายการจัดกลุ่มตามเดือน** — แต่ละกลุ่มเป็น `<section>` ที่มี `<h2>` ชื่อเดือน+ปีไทย
...
2. CardContent `space-y-4`: แถบ error OAuth (เมื่อ `?error=oauth`) → ปุ่ม Google เต็มกว้าง → เส้นคั่น "หรือ" → ฟอร์มอีเมล/รหัสผ่าน → ปุ่ม submit → ลิงก์ไป `/register`
...
1. **จำนวนปุ่มเมนู** — [DESIGN.md:42](../DESIGN.md#L42) เขียน "เมนูล่าง 4 ปุ่ม" · โค้ดจริงมี 5 ([nav-items.ts](../src/components/nav-items.ts)): เช็คอิน ภาพรวม เป้าหมาย โค้ช ตั้งค่า
2. **ขอบเขต SafetyNotice** — [docs/08-safety-privacy.md:15](08-safety-privacy.md#L15) เขียนว่าข้อความถาวรอยู่ "ใต้หน้า coach/dashboard" · โค้ดจริงแสดงใน 3 layout = แทบทุกหน้า (รวม landing และ login)

## พบจากสายเช็คอิน

3. **Breakpoint** — DESIGN.md ข้อ 3 ระบุ "breakpoint ที่ใช้จริงมีตัวเดียว: `lg`" และ "อย่าใช้ `sm:` / `md:` พร่ำเพรื่อ" · แต่ [checkin-history.tsx:162](../src/components/checkin/checkin-history.tsx#L162) ใช้ `sm:grid-cols-2 xl:grid-cols-3`
4. **skeleton ความกว้างไม่ตรงหน้าจริง** — `/checkin` ใช้ `width="content"` (1024px) แต่ [loading.tsx](../src/app/(app)/checkin/loading.tsx) ใช้ `FormSkeleton` ที่เป็น 448px → บนเดสก์ท็อปความกว้างจะกระตุกตอนโหลดเสร็จ
5. **skeleton รูปทรงไม่ตรงหน้าจริง** — `/checkin/history` เป็น grid 2–3 คอลัมน์บนจอกว้าง แต่ `loading.tsx` ใช้ `TextPageSkeleton` ที่เป็นการ์ดเรียงลงมาคอลัมน์เดียว
6. **copy ปุ่มผิดบริบทในหน้า backfill** — ปุ่มสุดท้ายเขียน "บันทึกเช็คอินวันนี้" hardcode ในแกน [checkin-form.tsx:637](../src/components/checkin/checkin-form.tsx#L637) → หน้า `/checkin/edit/[date]` ที่กรอกย้อนหลังก็ยังขึ้นคำว่า "วันนี้"
7. **`/checkin/edit/<วันที่ของวันนี้>` เข้าได้** — `days === 0` ผ่าน guard → ได้ฟอร์มหัวข้อ "บันทึกย้อนหลัง" สำหรับวันนี้ **โดยไม่มี guard ข้ามเที่ยงคืน**

## ช่องโหว่ a11y ที่พบ (สายเช็คอิน — ยังไม่ได้ตรวจสายอื่น)

8. **ไม่มี `aria-live` บนข้อความสถานะ** — "เลือกสักอันก่อนไปต่อนะ", บรรทัด error, ข้อความหลังลบ, คำถามยืนยันลบ ล้วนไม่ประกาศให้ screen reader · และ `goForward()` เลื่อนจอด้วย `scrollIntoView` โดยไม่ย้าย focus → ผู้ใช้คีย์บอร์ด/screen reader ไม่รู้ว่าติดตรงไหน
9. **กลุ่มชิปไม่มี `fieldset`/`legend` และ `<Label>` ไม่ผูก `htmlFor`** — [checkin-form.tsx:76](../src/components/checkin/checkin-form.tsx#L76) · มีแต่ textarea ที่ผูก id จริง
10. **nudge เป็น `<Link>` จัดสไตล์เอง ไม่ได้ใช้ `buttonVariants()`/`min-h-11`** — [page.tsx:26](../src/app/(app)/checkin/page.tsx#L26) ความสูงมาจาก padding + 2 บรรทัด (DESIGN.md เตือนเรื่องนี้ไว้ตรง ๆ ว่า "จุดนี้เคยหลุดมาแล้ว")

## พบจากสายภาพรวม + สรุปสัปดาห์

11. **`--chart-6` อยู่นอกช่วงที่เอกสารระบุ** — DESIGN.md ข้อ 4 เขียน "กราฟใช้ `--chart-1` ถึง `--chart-5`" · แต่แท็บ "กิน" ใช้ `--chart-6` กับเครื่องดื่มหวาน [pillar-charts.tsx:20](../src/components/dashboard/pillar-charts.tsx#L20) (token มีจริงใน globals.css พร้อมคู่ dark — แค่เอกสารไม่ได้อัปเดต)
12. **ใช้สี destructive กับข้อความชวนบันทึก** — ขัดกฎสี DESIGN.md ตรง ๆ ที่สงวน `--destructive` ให้ error ระบบเท่านั้น · กรณีข้อมูลไม่พอตอนกดปุ่มวิเคราะห์ ข้อความ "บันทึกต่ออีกนิดนะ" ไปโผล่ในกล่องแดง [generate-insight-button.tsx:28-31](../src/components/dashboard/generate-insight-button.tsx#L28)
13. **marker ปัจจัยรบกวนไม่มี focus ring** — `tabIndex={0}` แต่ไม่พบคลาส `focus-visible:` · เห็นวงแหวนก็ต่อเมื่อกด Enter แล้ว → คนใช้คีย์บอร์ด tab มาถึงแล้วไม่รู้ว่าอยู่ตรงไหน [disruptor-overlay.tsx:184-190](../src/components/dashboard/disruptor-overlay.tsx#L184) ขัดกฎ DESIGN.md ข้อ "Interactive ที่สร้างเองต้องมี focus ring"
14. **สเปก reflection ไม่ตรงกับของที่ลงจริง** — [F6-01](../.scratch/f6-weekly-reflection/issues/01-generation.md) เขียนว่า "generate เมื่อผู้ใช้เปิดดูสัปดาห์ที่จบแล้วและยังไม่มีรายงาน" · โค้ดจริงต้องกดปุ่มเอง (น่าจะจงใจเพื่อกันโควตา แต่ issue ไม่ได้อัปเดต)
15. **`day-lines.tsx` ใช้ `sm:` อีกจุด** — [day-lines.tsx:122](../src/components/dashboard/day-lines.tsx#L122) แถบ "ขยับ" เป็นหลอดเฉพาะ `sm:` ขึ้นไป (เพิ่มจากข้อ 3)
16. **`/reflection` ประกาศ `export const dynamic = "force-dynamic"`** โดยไม่มีคำอธิบายในโค้ดหรือ issue [page.tsx:20](../src/app/(app)/reflection/page.tsx#L20) — หน้าเดียวในแอปที่บังคับไว้ชัด ๆ

## พบจากสายโค้ช + เป้าหมาย

17. 🔴 **`text-[11px]` กับข้อความไทย — เป็นระเบิดเวลาของด่าน e2e** — [chat-client.tsx:739](../src/components/coach/chat-client.tsx#L739) คำว่า "คำถามแนะนำ:" เป็นไทย 11px ขัด DESIGN.md ข้อ 6 และ **ต่ำกว่าเกณฑ์ `expectNoTinyThai` ที่ตัดที่ 11.9px** · ที่ยังไม่แดงเพราะบล็อกนี้โผล่เฉพาะบัญชีที่ไม่มีประวัติแชท**และ**ไม่มี opener ซึ่งบัญชี demo ไม่เข้าเงื่อนไข → **ผู้ใช้ใหม่จริงจะเจอ และ e2e จะแดงทันทีถ้ามีใครเพิ่มเทสต์ด้วยบัญชีเปล่า**
18. **ข้อความโควตาหมดยังโผล่ในกล่องสีเตือนภัยได้** — `QuotaReachedNotice` ทำถูกแล้ว (muted + ไอคอน `Moon` ไม่มี destructive เลย) แต่ข้อความ "วันนี้คุยกับโค้ชครบ 5 ข้อความแล้ว…" ([actions.ts:88](../src/lib/chat/actions.ts#L88)) และ "โควตา AI ของวันนี้หมดแล้ว…" ([errors.ts:14](../src/lib/ai/errors.ts#L14)) ไหลเข้ากล่อง error ที่เป็น `border-destructive/20 bg-destructive/5 text-destructive` [chat-client.tsx:773](../src/components/coach/chat-client.tsx#L773) → ขัดกฎ DESIGN.md "โควตาหมด = โทน muted อบอุ่น ไม่ใช่ destructive"
19. **`##` ยังไม่ถูกแปลง** — AC ของ [F4-06](../.scratch/f4-coach/issues/06-markdown-bubbles.md) เขียนว่า "ฟองข้อความโค้ชไม่มี `**` `##` ดิบโผล่" · `FormattedMessage` แก้ `**` และ bullet แล้ว แต่ **ไม่ได้จัดการ `##`** → ถ้า Gemini ตอบเป็นหัวข้อจะเห็น `##` บนจอ (issue นี้ยังค้างใน BOARD อยู่แล้ว)
20. **คอมเมนต์ในโค้ดไม่ตรงกับพฤติกรรม** — [chat-client.tsx:204](../src/components/coach/chat-client.tsx#L204) เขียนว่า "เข้า guided flow เฉพาะตอนกดปุ่ม starter — ข้อความที่ผู้ใช้พิมพ์เองต้องถึงโค้ชเสมอ" แต่โค้ดเทียบ `text === GOAL_STARTER` เฉย ๆ → พิมพ์ "อยากตั้งเป้าสัปดาห์หน้า" เป๊ะ ๆ ก็เข้า flow ไม่ถึงโค้ช
21. **`md:` อีกจุด** — textarea ใช้ `text-base md:text-sm` [chat-client.tsx:827](../src/components/coach/chat-client.tsx#L827) (เพิ่มจากข้อ 3, 15)
22. **`aria-label` บน `<div>` เปล่า** — `PendingReply` ใส่ `aria-label="โค้ชกำลังคิด"` บน div ที่ไม่มี role [message-variants.tsx:43](../src/components/coach/message-variants.tsx#L43) → ตามสเปกจะไม่ถูก expose จุด 3 จุดเด้งอาจไม่ถูกอ่าน
23. **Suspense ที่ไม่ได้ defer อะไรจริง** — `/goals` ครอบ `GoalSuggestionCard` ด้วย Suspense [page.tsx:32](../src/app/(app)/goals/page.tsx#L32) แต่การ์ดนั้นเป็น client component ที่ไม่ await อะไรของตัวเอง (ข้อมูล await ที่ระดับหน้าไปแล้ว) → fallback แทบไม่มีโอกาสแสดง · ส่วน `GoalProgressCard` ไม่ถูกครอบเลย
24. **`aria-label` เป็น ISO ดิบ** — ช่องติ๊กวันใน `/goals` ใช้ `aria-label="2026-07-22"` [goal-progress-card.tsx:95](../src/components/goals/goal-progress-card.tsx#L95) ทั้งที่โปรเจกต์มี `formatThaiDate()` อยู่แล้ว
25. **ปุ่ม "ข้อที่ 1 / ข้อที่ 2" ไม่มี `aria-pressed`** [goal-suggestion-card.tsx:136](../src/components/goals/goal-suggestion-card.tsx#L136) ทั้งที่การ์ดตัวเลือกใน guided flow มี
26. **ลำดับหัวข้อข้าม h2** — `/goals` มี h1 แล้วกระโดดไป `<h3>` "เป้าหมายที่กำลังทำ" [page.tsx:39](../src/app/(app)/goals/page.tsx#L39)
27. **skeleton ไม่ตรงทรงอีก 2 หน้า** — `/goals` ใช้ `TextPageSkeleton` (การ์ดข้อความ 3 ใบ) แต่หน้าจริงเป็นการ์ดขอเป้า + ตารางติ๊ก 7 ช่อง · `/coach` loading ใช้ `17.5rem` แต่หน้าจริง `17.75rem` + หัก safe-area (เพิ่มจากข้อ 4, 5)
28. **เป้าที่ทำสำเร็จหายไปเงียบ ๆ** — กด "ทำเป้านี้สำเร็จ" แล้วการ์ดหายจากหน้า ไม่มีส่วน "เป้าที่ทำสำเร็จแล้ว" · ข้ามสัปดาห์เป้าเก่าก็หายเองโดยไม่มีข้อความอธิบายว่าไปไหน — **ไม่พบในโค้ด** (อาจตั้งใจ แต่ไม่มีหลักฐานว่าตั้งใจ)

## พบจากสายทางเข้า + ตั้งค่า + หน้าพัง

29. 🔴 **ลบบัญชีแล้วผู้ใช้ไม่ได้รับการยืนยันอะไรเลย** — `deleteAccount()` เด้งไป `/login?deleted=1` [actions.ts:54](../src/lib/account/actions.ts#L54) แต่หน้า login **อ่านเฉพาะ `error` ไม่อ่าน `deleted`** → ผู้ใช้เพิ่งลบบัญชีถาวรแล้วเจอหน้า login เปล่า ๆ ไม่รู้ว่าสำเร็จหรือหลุด session
30. **h1 ของ `/login` และ `/register` ผิดสเกล** — ซ้อนอยู่ใน `CardTitle` จึงได้ `text-base font-medium` แทนที่จะเป็น `text-xl font-semibold lg:text-2xl` ตามสเกล DESIGN.md
31. **ไม่มีการ log error ที่ไหนเลย** — [error.tsx](../src/app/error.tsx) และ [(app)/error.tsx](../src/app/(app)/error.tsx) รับ prop `error: Error` แต่ destructure เฉพาะ `reset` → **ระบบพังบน production แล้วไม่มีร่องรอย**
32. **ไม่มี `global-error.tsx`** — error ที่เกิดใน root layout ตกไปที่หน้า default ของ Next (ภาษาอังกฤษ ไม่มีแบรนด์)
33. **ไม่มี UI แก้โปรไฟล์เลยทั้งแอป** — [F0-02](../.scratch/f0-account-onboarding/issues/02-onboarding-profile.md) ติ๊กค้างว่าเลื่อนไป F7 แต่ไม่เคยทำ → ผู้ใช้แก้ชื่อเล่น/สถานะ/ข้อจำกัดไม่ได้หลัง onboarding
34. **skeleton ไม่ตรงทรงอีกจุด** — `/settings/privacy` ใช้ `TextPageSkeleton` 3 การ์ด ทั้งที่หน้าจริงมี 7 การ์ด (เพิ่มจากข้อ 4, 5, 30)
35. **`DeleteZone` ไม่มี `role="alert"`** [delete-zone.tsx:98-103](../src/components/settings/delete-zone.tsx#L98) — ต่างจาก `AuthMessage` ของหน้า auth ที่มี · ลบพลาดแล้ว screen reader ไม่ถูกประกาศ **ทั้งที่เป็นการกระทำที่ย้อนไม่ได้**
36. **onboarding: progressbar ไม่มี `aria-label`** — มี `role="progressbar"` + `aria-valuenow/min/max` ครบแต่ไม่มีชื่อ
37. **onboarding: checkbox รับทราบไม่มีสไตล์ focus ที่มองเห็น** — span มีแค่ `has-checked:` ไม่มี `has-focus-visible:` → **คนใช้คีย์บอร์ด tab มาถึงช่องติ๊ก disclaimer แล้วไม่รู้ว่าอยู่ตรงไหน** (ขัดกฎ DESIGN.md เรื่อง focus ring และเป็นด่านบังคับ FR-0.3)
38. **onboarding: กลุ่ม Chip ไม่ได้ผูกกับ label** — ไม่มี `role="group"` / `aria-labelledby` (อาการเดียวกับข้อ 9 ในหน้าเช็คอิน)
39. **landing / login / register ไม่มี `loading.tsx`** — DESIGN.md บอก "ทุกหน้าใหม่ต้องมี `loading.tsx` ห้ามลืม" · หน้าเหล่านี้เบาและ redirect เร็ว ผลกระทบน้อย แต่ก็เป็นข้อยกเว้นที่ไม่ได้บันทึกไว้

## เรื่องที่ผู้ใช้ไม่มีทางรู้ (ไม่ใช่บั๊ก แต่ควรรู้ตอนตอบกรรมการ)

40. **โควตา Gemini หมด = ผู้ใช้ไม่รู้เลย** — ทั้ง insight และ reflection fallback ไปใช้ template เงียบ ๆ ตัวเลขยังจริงเพราะมาจาก `lib/patterns` · ข้อความ "โควตา AI ของวันนี้หมดแล้ว…" ใน [errors.ts:15](../src/lib/ai/errors.ts#L15) **ไม่ถูกใช้บน 2 หน้านี้** — เป็นการตัดสินใจที่บันทึกไว้ใน [F3-03](../.scratch/f3-pattern-analysis/issues/03-insight-endpoint-cache.md) ("Gemini ล่ม/โควตาหมด → template ทั้งหมด หน้าไม่พัง")
41. **cache hit ตอนกดปุ่มก็ไม่มีข้อความบอก** — กด "วิเคราะห์ใหม่" แล้วได้ของเดิม ผู้ใช้เห็นแค่หน้า refresh
42. **DB error ตอนดึง checkin ถูกกลืน** — `getCheckins` คืน `[]` [queries.ts:15](../src/lib/checkins/queries.ts#L15) → ผู้ใช้เห็นหน้า "ยังไม่มีข้อมูลสุขภาพ" เหมือนผู้ใช้ใหม่ ทั้งที่จริงคือระบบพัง

## หมายเหตุ

- ข้อ 8–10 เป็น **ช่องโหว่จริงในโค้ด** ไม่ใช่แค่เอกสารไม่ตรง — แต่ e2e ปัจจุบันไม่จับ เพราะด่านตรวจดู contrast/ขนาด/h1 ไม่ได้ตรวจ `aria-live` หรือ focus management
- ข้อ 4–7 เป็นของเล็ก แก้ได้เร็ว แต่ **ไม่ควรแก้ตอนนี้** (เหลือ 7 วันถึง code freeze คิวคือ QA)
