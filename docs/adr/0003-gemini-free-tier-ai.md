...
- **หมายเหตุเรื่อง "สลับ provider":** เจตนาเดิม (บรรทัด 14, 21) คือให้ `lib/ai` เป็นชั้นกลางที่ย้าย provider ได้ · ที่ลงจริงคือ **ประตูเดียว + สลับ *รุ่น* ได้ด้วย env `AI_MODEL`** ส่วนการย้ายไป Typhoon/provider อื่นยังต้องแก้ `lib/ai/index.ts` เอง (ผูกกับ `@google/genai`) — พอสำหรับแผนรับมือ rate limit เพราะรุ่นสำรองใน Gemini ก็สลับได้แล้ว
